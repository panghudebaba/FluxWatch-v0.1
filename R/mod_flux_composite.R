...
fw_composite_get_daily_all <- function(input, key, step1_qf_wq) {
  ...
  tryCatch({
    # ---- 1. 检查指标列是否存在 ----
    if (!(con %in% names(wq_df))) return(NULL)

    # ---- 2. 解析日期 ----
    qf_df$date <- fw_as_date(qf_df$TM)
    wq_df$date <- fw_as_date(wq_df$TM)

    # ---- 3. 构建 QF 子集（date + Q + 站点信息）----
    qf_sub <- data.frame(
      date = qf_df$date,
      Q    = fw_as_num(qf_df$Q),
      stringsAsFactors = FALSE
    )
    if ("WYBM" %in% names(qf_df))
      qf_sub$station <- as.character(qf_df$WYBM)
    if ("STNM" %in% names(qf_df))
      qf_sub$STNM <- as.character(qf_df$STNM)
    qf_sub <- qf_sub[!is.na(qf_sub$date), , drop = FALSE]

    # ---- 4. 构建 WQ 子集（date + 选中指标 → 重命名为 C）----
    wq_sub <- data.frame(
      date = wq_df$date,
      C    = fw_as_num(wq_df[[con]]),
      stringsAsFactors = FALSE
    )
    wq_sub <- wq_sub[!is.na(wq_sub$date), , drop = FALSE]

    # ---- 5. 按日期合并（左连接保留全部流量日）----
    merged <- merge(qf_sub, wq_sub, by = "date", all.x = TRUE)

    if (nrow(merged) == 0) return(NULL)

    # ---- 6. 传入合并后的单一 data.frame ----
    fw_prepare_flux_data(merged)
